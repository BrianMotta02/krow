...
function PriorityBadge({ priority }: { priority: TaskPriority }) {
  return (
    <View style={[styles.priorityBadge, { backgroundColor: PRIORITY_COLORS[priority] + '22', borderColor: PRIORITY_COLORS[priority] }]}>
      <Text style={[styles.priorityText, { color: PRIORITY_COLORS[priority] }]}>● {PRIORITY_LABELS[priority]}</Text>
    </View>
  );
}

export default function Tarefas() {
  const { tasks, addTask, updateStatus, toggleChecklistItem, addChecklistItem, addComment, getTask } = useTasks();
  const { projects } = useProjects();
  const { user } = useAuth();

  const [activeTab, setActiveTab] = useState<'a_fazer' | 'em_andamento' | 'concluida'>('a_fazer');
  ...
  function handleAddTask() {
    ...
    addTask({ title: titulo, description: descricao, projectId, projectName, assignedTo: responsavel, priority, deadline });
    setTitulo(''); setDescricao(''); setProjectId(''); setProjectName('');
    setResponsavel(''); setPriority('media'); setDeadline('');
    setModalAdd(false);
  }
  ...
  const tabs: { key: 'a_fazer' | 'em_andamento' | 'concluida'; label: string }[] = [
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <TextInput style={styles.input} placeholder="Título da tarefa" placeholderTextColor="#b0b5c8" value={titulo} onChangeText={setTitulo} />

              <Text style={styles.fieldLabel}>Descrição:</Text>
              <TextInput style={[styles.input, { height: 80 }]} placeholder="Descrição da tarefa" placeholderTextColor="#b0b5c8" value={descricao} onChangeText={setDescricao} multiline />
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 12 }}>
                  {projects.map(p => (
                    <TouchableOpacity
                      ...
                      onPress={() => { setProjectId(p.id); setProjectName(p.name); }}
                    >
              ...
              <TextInput style={styles.input} placeholder="Nome do responsável" placeholderTextColor="#b0b5c8" value={responsavel} onChangeText={setResponsavel} />
              ...
              <TextInput style={styles.input} placeholder="Ex: 12/06/2026" placeholderTextColor="#b0b5c8" value={deadline} onChangeText={setDeadline} />

              <TouchableOpacity style={styles.btnConfirmar} onPress={handleAddTask}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            {selectedTask && (
              <>
                <View style={styles.modalHeader}>
                  <Text style={styles.editTitle} numberOfLines={1}>{selectedTask.title}</Text>
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                  ...
                  <View style={styles.badgesRow}>
                    ...
                    <View style={styles.projectBadge}>
                      <Text style={styles.projectBadgeText}>{selectedTask.projectName}</Text>
                    </View>
                    <View style={styles.statusBadge}>
                      <Text style={styles.statusBadgeText}>{selectedTask.status.replace('_', ' ').toUpperCase()}</Text>
                    </View>
                  </View>

                  {/* Descrição */}
                  <Text style={styles.sectionTitle}>DESCRIÇÃO</Text>
                  <Text style={styles.descText}>{selectedTask.description || 'Sem descrição.'}</Text>

                  {/* Prazo */}
                  <View style={styles.infoRow}>
                    <View style={styles.infoBox}>
                      <Text style={styles.infoLabel}>PRAZO</Text>
                      <Text style={styles.infoValue}>📅 {selectedTask.deadline}</Text>
                    </View>
                    <View style={styles.infoBox}>
                      <Text style={styles.infoLabel}>CRIADA EM</Text>
                      <Text style={styles.infoValue}>{selectedTask.createdAt.toLocaleDateString('pt-BR')}</Text>
                    </View>
                  </View>
                  ...
                  <View style={styles.assigneeRow}>
                    <View style={styles.assigneeAvatar}>
                      <Text style={styles.assigneeAvatarText}>
                        {selectedTask.assignedTo.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                      </Text>
                    </View>
                    <Text style={styles.assigneeName}>{selectedTask.assignedTo}</Text>
                  </View>

                  {/* Checklist */}
                  <Text style={styles.sectionTitle}>
                    CHECKLIST ({selectedTask.checklist.filter(c => c.done).length} de {selectedTask.checklist.length} concluídos)
                  </Text>
                  ...
                  <View style={styles.addRow}>
                    ...
                    />
                    <TouchableOpacity style={styles.addBtn} onPress={handleAddCheckItem}>
                      <Text style={styles.addBtnText}>+</Text>
                    </TouchableOpacity>
                  </View>

                  {/* Comentários */}
                  <Text style={styles.sectionTitle}>COMENTÁRIOS ({selectedTask.comments.length})</Text>
                    ...
                    <View key={c.id} style={styles.commentCard}>
                      <View style={styles.commentHeader}>
                        ...
                        <View>
                          ...
                          <Text style={styles.commentDate}>{c.createdAt.toLocaleDateString('pt-BR')}</Text>
                        </View>
                      </View>
                  ...
                  <View style={styles.actionsRow}>
                    ...
                    >
                      <Text style={styles.btnConcluirText}>Marcar concluída</Text>
                    </TouchableOpacity>
                  </View>
                </ScrollView>
              </>
...
const styles = StyleSheet.create({
  ...
  pageTitle: { fontSize: 20, fontWeight: '900', color: '#1a2540', marginBottom: 2 },
  pageSub: { fontSize: 12, color: '#7a8099', marginBottom: 16 },
  tabsRow: { flexDirection: 'row', gap: 8, marginBottom: 20 },
  tabBtn: { flex: 1, paddingVertical: 8, borderRadius: 8, backgroundColor: '#fff', borderWidth: 1.5, borderColor: '#dde0ea', alignItems: 'center' },
  ...
  taskCard: { backgroundColor: '#fff', borderRadius: 14, padding: 14, borderWidth: 1.5, borderColor: '#dde0ea' },
  taskCardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  taskTitle: { fontSize: 14, fontWeight: '800', color: '#1a2540', marginBottom: 4 },
  taskDesc: { fontSize: 12, color: '#7a8099', marginBottom: 10, lineHeight: 18 },
  ...
  fab: { position: 'absolute', bottom: 90, right: 20, width: 56, height: 56, borderRadius: 28, backgroundColor: '#2e7de1', alignItems: 'center', justifyContent: 'center', elevation: 6 },
  fabIcon: { color: '#fff', fontSize: 30, fontWeight: '300', lineHeight: 34 },
  // Modal
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.3)' },
  modalContent: { flex: 1, backgroundColor: '#f2f3f7', marginTop: 48, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20 },
  ...
  fieldLabel: { fontSize: 13, fontWeight: '700', color: '#1a2540', marginBottom: 6, marginTop: 12 },
  input: { backgroundColor: '#fff', borderRadius: 10, borderWidth: 1.5, borderColor: '#dde0ea', paddingHorizontal: 16, paddingVertical: 13, fontSize: 14, color: '#1a2540', marginBottom: 0 },
  projectChip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, borderWidth: 1.5, borderColor: '#dde0ea', backgroundColor: '#fff', marginRight: 8 },
  ...
  priorityChip: { flex: 1, paddingVertical: 8, borderRadius: 8, borderWidth: 1.5, borderColor: '#dde0ea', alignItems: 'center', backgroundColor: '#fff' },
  priorityChipText: { fontSize: 12, fontWeight: '700', color: '#7a8099' },
  btnConfirmar: { backgroundColor: '#2e7de1', borderRadius: 10, paddingVertical: 15, alignItems: 'center', marginTop: 24, marginBottom: 32 },
  btnConfirmarText: { color: '#fff', fontSize: 16, fontWeight: '800' },
  // Editar tarefa
  editTitle: { fontSize: 16, fontWeight: '900', color: '#1a2540', flex: 1, marginRight: 12 },
