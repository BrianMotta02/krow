...
import {
  ...
  ScrollView, SafeAreaView, Modal, TextInput, Alert,
} from 'react-native';
import Svg, { Circle } from 'react-native-svg';
import { useProjects } from '../../context/ProjectContext';

function DonutChart({ percent, size = 70 }: { percent: number; size?: number }) {
  const strokeWidth = 8;
...
export default function Projetos() {
  ...
  function handleAddProject() {
    ...
    addProject({ name: nome, description: descricao, manager: gerente, startDate: dataInicio, endDate: dataEntrega });
    setNome(''); setDescricao(''); setDataInicio(''); setDataEntrega(''); setGerente('');
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* Botão ver todos */}
        <TouchableOpacity style={styles.btnVerTodos} onPress={() => setModalVerTodos(true)}>
        ...
        <View style={styles.list}>
          {projects.length === 0 ? (
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>Use o botão + para adicionar.</Text>
            </View>
          ) : (
            projects.map(p => (
      ...
      {/* FAB */}
      <TouchableOpacity style={styles.fab} onPress={() => setModalAdicionar(true)}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.grid}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => { setModalVerTodos(false); setModalAdicionar(true); }}
                >
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <TextInput style={styles.input} placeholder="Digite o nome do projeto" placeholderTextColor="#b0b5c8" value={nome} onChangeText={setNome} />

              <Text style={styles.fieldLabel}>Data de início:</Text>
              <TextInput style={styles.input} placeholder="Digite a data de início" placeholderTextColor="#b0b5c8" value={dataInicio} onChangeText={setDataInicio} />

              <Text style={styles.fieldLabel}>Data de entrega:</Text>
              <TextInput style={styles.input} placeholder="Digite a data de entrega" placeholderTextColor="#b0b5c8" value={dataEntrega} onChangeText={setDataEntrega} />

              <Text style={styles.fieldLabel}>Gerente do projeto:</Text>
              <TextInput style={styles.input} placeholder="Digite quem é o gerente do projeto" placeholderTextColor="#b0b5c8" value={gerente} onChangeText={setGerente} />

              <TouchableOpacity style={styles.btnConfirmar} onPress={handleAddProject}>
...
const styles = StyleSheet.create({
  ...
  btnVerTodos: { backgroundColor: '#2e7de1', borderRadius: 10, paddingVertical: 12, alignItems: 'center', marginBottom: 24 },
  ...
  projectRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 14, padding: 14, gap: 14, borderWidth: 1.5, borderColor: '#dde0ea' },
  ...
  projectInfo: { flex: 1 },
  projectName: { fontSize: 16, fontWeight: '900', color: '#1a2540', marginBottom: 2 },
  ...
  gridCard: { width: '47%', backgroundColor: '#fff', borderRadius: 14, padding: 14, borderWidth: 1.5, borderColor: '#dde0ea', alignItems: 'center' },
  gridCardAdd: { alignItems: 'center', justifyContent: 'center', backgroundColor: '#2e7de1' },
  gridAddIcon: { fontSize: 40, color: '#fff', fontWeight: '300' },
  gridName: { fontSize: 13, fontWeight: '900', color: '#1a2540', marginBottom: 8, textAlign: 'center' },
  gridDonutWrap: { position: 'relative', alignItems: 'center', justifyContent: 'center', marginBottom: 6 },
  gridDonutLabel: { position: 'absolute', fontSize: 14, fontWeight: '900', color: '#1a2540' },
  gridDates: { fontSize: 10, color: '#7a8099' },
  membersRow: { flexDirection: 'row', marginTop: 8 },
  memberAvatar: { width: 26, height: 26, borderRadius: 13, backgroundColor: '#1e5fc2', alignItems: 'center', justifyContent: 'center', borderWidth: 1.5, borderColor: '#fff' },
